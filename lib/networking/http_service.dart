...
class HttpService<T, S extends Serializable<T>> {
  ...
  Future<T> get(String path) async {
    var responseObject;
    try {
      final response = await http.get(FlavorConfig.getBaseUrl() + path);
  ...
  Future<T> post(String path, T t) async {
    var responseObject;
    try {
      final response = await http.post(FlavorConfig.getBaseUrl() + path,
  ...
  Future<T> put(String path, T t) async {
    var responseObject;
    try {
      final response = await http.put(FlavorConfig.getBaseUrl() + path,
          body: jsonEncode(_serializable.toJson(t)),
          headers: _getDefaultHeaders());
      responseObject = _processResponse(response);
    } catch (e) {
  ...
  Future<bool> delete(String path) async {
    var result;
    try {
      final response = await http.delete(FlavorConfig.getBaseUrl() + path);
