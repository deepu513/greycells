...
class AddressDetailEditPage extends StatelessWidget implements Validatable {
  ...
  FutureOr<bool> validate(BuildContext context, ValidationBloc validationBloc) {
    ...
    subscription = validationBloc.listen((validationState) {
      if (validationState is AddressFieldsValid) {
        BlocProvider.of<PatientDetailsEditBloc>(context).add(AddressValidated());
...
class GuardianAddressInput extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<PatientDetailsEditBloc, PatientDetailsEditState>(
